...
class MatchPair(torch.utils.data.Dataset):
	def __init__(self,
		...
		if subset == 'train':
			...
			All_list_file = open(os.path.join(self.base_dir, 'train_adj_2.txt'))
		else:
			self.data_augmentation = False
			All_list_file = open(os.path.join(self.base_dir, 'test_adj_2.txt'))
			self.keep_ratio = -1.0
	...
	def __getitem__(self, index):
		...
		src_pcd = np.loadtxt(src_path).astype(np.float32)
		tgt_pcd = np.loadtxt(tgt_path).astype(np.float32)
		...
		if self.return_normals:
			src_norm_path = src_path.split('.')[0] + '_norm.txt'
			tgt_norm_path = tgt_path.split('.')[0] + '_norm.txt'

			tgt_pcd_norm = np.loadtxt(tgt_norm_path).astype(np.float32)
			src_pcd_norm = np.loadtxt(src_norm_path).astype(np.float32)
		...
		if self.return_normals:
			...
			src_move = apply_transform(src_move, inv_transform_src, normals=src_pcd_norm)
			tgt_move = apply_transform(tgt_move, transform_ref, normals=tgt_pcd_norm)
...
class MatchGraph(BasePCGraph):
	def __init__(self, dataset_root='Data/Global_Collect09', subset='train', class_indices='', return_normals=True,
...
if __name__ == '__main__':
	...
	Dataset = MatchPair(
	             ...
	             # dataset_root='Data/office1'
				dataset_root = 'Data/Global_Collect09',
				# dataset_type='adj_12'
	             )
	# sun3d   45115/11590
	# 7-scenes 1268/548
	...
	for d in Dataset:
		...
		# pdb.set_trace()
		utils.vis_PC([d['ref_points'], d['src_points']], point_size=6.)
